...
data$sm3 <- as.numeric(factor(data$sm3, 
                              levels = c("Non", "Oui", "Je ne comprends pas"), 
                              exclude = NULL))
...
data$cv1 <- as.numeric(factor(data$cv1, 
                              levels = c("Plus de 2h par jour", "Entre 1h et 1h30 par jour","30 minutes par jour ou moins","Aucun","Entre 30 minutes et 1h par jour", "Entre 1h30 et 2h par jour"), exclude = NULL))
...
data$cv2a <- as.numeric(factor(data$cv2a, 
                              levels = c("Jamais"   ,      "6 fois ou plus", "1 fois"    ,     "2 ou 3 fois" ,   "4 ou 5 fois"), 
                              exclude = NULL))
...
data$cv2b <- as.numeric(factor(data$cv2b, 
                              levels = c("Jamais"  ,       "2 ou 3 fois"   , "6 fois ou plus" ,"1 fois"  , "4 ou 5 fois"  ), 
                              exclude = NULL))
...
data$cv3 <- as.numeric(factor(data$cv3, 
                              levels = c("Non", "Oui meme sans permis", "Oui seulement apres avoir eu mon permis"),
                              exclude = NULL))
...
data$cv4a <- as.numeric(factor(data$cv4a, 
                              levels = c("6 fois ou plus", "Jamais"       ,  "1 fois"        
, "2 ou 3 fois"   , "4 ou 5 fois"), 
                              exclude = NULL))
...
"Toujours"                                      ,
 "La plupart du temps"                           ,
 "Je ne suis pas monte dans un vehicule a moteur"), 
                            exclude = NULL))
...
data$tb4 <- as.numeric(factor(data$tb4, 
                              levels = c("Tous les jours", "1 a 2 jours", "10 a 19 jours", "3 a 5 jours", "20 a 29 jours", "6 a 9 jours"), 
                              exclude = NULL))
...
data$tb5 <- as.numeric(factor(data$tb5, 
                              levels = c("1 cigarette par jour", "Moins d'une cigarette par jour", "11 a 20 cigarettes par jour", "2 a 5 cigarettes par jour", "Plus de 20 cigarettes par jour", "6 a 10 cigarettes par jour"
), 
                              exclude = NULL))
...
data$tb8 <- as.numeric(factor(data$tb8, 
                              levels = c("0 jour", "Tous les jours", "1 ou 2 jours", "6 a 9 jours", "3 a 5 jours", "10 a 19 jours", "20 a 29 jours"), 
                              exclude = NULL))
...
data$ao1 <- as.numeric(factor(data$ao1, 
                              levels = c("Non", "Oui, plus que quelques gorgees", "Oui, juste une ou deux gorgees pour gouter/tester ou sans faire expres"
), 
                              exclude = NULL))
...
data$ao2a <- as.numeric(factor(data$ao2a, 
                              levels = c("10 ans", "11 ans", "13 ans", "12 ans", "9 ans", "15 ans", "7 ans ou moins", "17 ans", "16 ans", "14 ans", "8 ans", "18 ans ou plus"
), 
                              exclude = NULL))
...
data$ao2b <- as.numeric(factor(data$ao2b, 
                              levels = c("7 ans ou moins", "12 ans", "11 ans", "10 ans", "14 ans", "15 ans", "16 ans", "17 ans", "18 ans ou plus", "13 ans", "8 ans", "9 ans"
), 
                              exclude = NULL))
...
data$ao3a <- as.numeric(factor(data$ao3a, 
                              levels = c("Jamais", "Rarement", "Chaque mois", "Chaque jour", "Chaque semaine"
), 
                              exclude = NULL))
...
data$ao3b <- as.numeric(factor(data$ao3b, 
                              levels = c("Jamais", "Rarement", "Chaque mois", "Chaque semaine", "Chaque jour"
), 
                              exclude = NULL))
...
data$ao3c <- as.numeric(factor(data$ao3c, 
                              levels = c("Chaque jour", "Jamais", "Chaque mois", "Chaque semaine", "Rarement"
), 
                              exclude = NULL))
...
data$ao3d <- as.numeric(factor(data$ao3d, 
                              levels = c("Jamais", "Chaque mois", "Rarement", "Chaque semaine", "Chaque jour"
), 
                              exclude = NULL))
...
data$ao5 <- as.numeric(factor(data$ao5, 
                              levels = c("1 a 2 jours", "6 a 9 jours", "3 a 5 jours", "20 a 29 jours", "10 a 19 jours", "Tous les jours"), 
                              exclude = NULL))
...
data$ao6 <- as.numeric(factor(data$ao6, 
                              levels = c("Moins d'un verre", "1 verre", "4 verres", "3 verres", "5 verres ou plus", "2 verres"
), 
                              exclude = NULL))
...
data$ao7 <- as.numeric(factor(data$ao7, 
                              levels = c("0 fois", "1 ou 2 fois", "20 fois ou plus", "3 a 9 fois", "10 a 19 fois"
), 
                              exclude = NULL))
...
data$ao10 <- as.numeric(factor(data$ao10, 
                              levels = c("11 ans", "15 ans", "16 ans", "17 ans", "10 ans", "13 ans", "7 ans ou moins", "14 ans", "12 ans", "8 ans", "18 ans ou plus"), 
                              exclude = NULL))
...
data$ao11 <- as.numeric(factor(data$ao11, 
                              levels = c("1 ou 2 fois", "3 a 9 fois", "0 fois", "10 fois ou plus"
), 
                              exclude = NULL))
...
data$cn2 <- as.numeric(factor(data$cn2, 
                              levels = c("7 ans ou moins", "10 ans", "17 ans", "14 ans", "13 ans", "16 ans", "15 ans", "11 ans", "12 ans", "18 ans ou plus", "9 ans", "8 ans"
), 
                              exclude = NULL))
...
data$cn4 <- as.numeric(factor(data$cn4, 
                              levels = c("1 a 2 jours", "10 a 19 jours", "Tous les jours", "3 a 5 jours", "6 a 9 jours", "20 a 29 jours"), 
                              exclude = NULL))
...
data$cn6 <- as.numeric(factor(data$cn6, 
                              levels = c("0 fois"     ,     "1 ou 2 fois" ,    "3 a 9 fois"   ,   "10 a 19 fois" ,   "20 fois ou plus"), 
                              exclude = NULL))
...
data$ad1 <- as.numeric(factor(data$ad1, 
                              levels = c("Non"        ,                      "Je ne sais pas ce qu'est le kava" ,"Oui" ), 
                              exclude = NULL))
...
, "Des personnes du sexe oppose"                                  
, "Je prefere ne pas repondre"                                    
, "Par des personnes du sexe oppose ET des personnes du meme sexe"
, "Je ne suis pas sure"                                           
, "Des personnes du meme sexe"    ), 
                              exclude = NULL))
...
data$ss2 <- as.numeric(factor(data$ss2, 
                              levels = c("Non", "Je prefere ne pas repondre", "Je ne comprends pas la question", "Oui"
), 
                              exclude = NULL))
...
data$ss4 <- as.numeric(factor(data$ss4, 
                              levels = c("Non", "Oui", "Je ne sais pas"
), 
                              exclude = NULL))
...
data$ss10 <- as.numeric(factor(data$ss10, 
                              levels = c("Non"      ,                  "Je prefere ne pas repondre" ,"Oui"    ), 
                              exclude = NULL))
...
data$ss11 <- as.numeric(factor(data$ss11, 
                              levels = c("Non", "Je ne suis pas sur", "Je ne sais pas ce qu'est le porno", "Oui"
), 
                              exclude = NULL))
...
data$pr1a <- as.numeric(factor(data$pr1a, 
                              levels = c("Plutot bien informe", "Je ne sais pas", "Plutot mal informe", "Je n'en ai jamais entendu parler"
), 
                              exclude = NULL))
...
data$pr1c <- as.numeric(factor(data$pr1c, 
                              levels = c("Plutot bien informe", "Je ne sais pas", "Je n'en ai jamais entendu parler", "Plutot mal informe"
), 
                              exclude = NULL))
...
data$pr1d <- as.numeric(factor(data$pr1d, 
                              levels = c("Je ne sais pas", "Plutot bien informe", "Je n'en ai jamais entendu parler", "Plutot mal informe"
), 
                              exclude = NULL))
...
data$pr1e <- as.numeric(factor(data$pr1e, 
                              levels = c("Je n'en ai jamais entendu parler", "Je ne sais pas", "Plutot bien informe", "Plutot mal informe"
), 
                              exclude = NULL))
...
data$pr2a <- as.numeric(factor(data$pr2a, 
                              levels = c("Je ne connais pas le risque", "Je ne sais pas ce que c'est", "Risque plutot faible", "Risque plutot eleve", "Risque tres eleve", "Risque nul"
), 
                              exclude = NULL))
...
data$pr2b <- as.numeric(factor(data$pr2b, 
                              levels = c("Risque plutot eleve", "Je ne sais pas ce que c'est", "Risque plutot faible", "Risque tres eleve", "Risque nul", "Je ne connais pas le risque"
), 
                              exclude = NULL))
...
data$pr2c <- as.numeric(factor(data$pr2c, 
                              levels = c("Risque nul", "Je ne sais pas ce que c'est", "Risque plutot faible", "Risque plutot eleve", "Risque tres eleve", "Je ne connais pas le risque"
), 
                              exclude = NULL))
...
data$pr2d <- as.numeric(factor(data$pr2d, 
                              levels = c("Je ne connais pas le risque", "Je ne sais pas ce que c'est", "Risque plutot faible", "Risque tres eleve", "Risque nul", "Risque plutot eleve"
), 
                              exclude = NULL))
...
data$pr2g <- as.numeric(factor(data$pr2g, 
                              levels = c("Risque tres eleve", "Je ne sais pas ce que c'est", "Risque plutot faible", "Je ne connais pas le risque", "Risque nul", "Risque plutot eleve"
), 
                              exclude = NULL))
...
data$pr2h <- as.numeric(factor(data$pr2h, 
                              levels = c("Je ne connais pas le risque", "Je ne sais pas ce que c'est", "Risque plutot faible", "Risque tres eleve", "Risque plutot eleve", "Risque nul"),
                              exclude = NULL))
