...
# --------------------- SCRIPT FINAL log_reg UNIQUEMENT --------------------- #
library(tidymodels)
library(data.table)
library(readxl)
library(dplyr)
...
# Entraînement log_reg
train_log_reg_model <- function(target_column, train, test) {
  ...
  # Pas de tuning car pas de paramètre à tuner
  fit(wf, data = train)
}
...
")
  model <- train_log_reg_model(col, train, test)
  ...
  saveRDS(model, paste0("log_reg_model_", col, ".rds"))
}
results
